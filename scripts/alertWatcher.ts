...
// Token Akses Utama Gerbang API Fonnte Anda
...
rtdb.ref('sensorLive').on('value', async (snapshot) => {
  ...
  const jedaEnamMenit = 6 * 60 * 1000; // Jeda waktu tunggu 6 menit

  for (const [sensorId, telemetry] of Object.entries(nodes)) {
    const currentStatus = telemetry.status; 

    // A. JIKA SENSOR SUDAH AMAN (SAFE)
    if (currentStatus === 'safe') {
      // Jika kondisi kembali aman, hapus catatan waktu agar jika nanti bocor lagi bisa langsung kirim instan
    ...
    // B. LOGIKA TUNGGU 6 MENIT
    const lastSentTime = lastNotificationTimeMap.get(sensorId) || 0;
    
    if (lastSentTime > 0 && (currentTime - lastSentTime) < jedaEnamMenit) {
      // Masih dalam masa tunggu 6 menit, abaikan pengiriman detik ini
      continue;
    }

    try {
      // C. FIX ERROR INDEX FIRESTORE: Cari alert aktif yang belum ditangani tanpa menggunakan orderBy()
      const activeAlertsQuery = await db.collection('alerts')
        .where('sensorId', '==', sensorId)
        .where('isResolved', '==', false)
        .get();

      // JIKA INI RENCANA PENGINGAT ULANG (setelah 6 menit berlalu), TAPI SUDAH DIKLIK "TANGANI" DI WEB
      if (lastSentTime > 0 && activeAlertsQuery.empty) {
        console.log(`[${sensorId}] Deteksi masih ${currentStatus}, tetapi sudah diklik 'Tangani' di web. Pengiriman dibatalkan.`);
        lastNotificationTimeMap.delete(sensorId); // Reset tracking waktu
        continue;
      }

      // D. PROSES KIRIM NOTIFIKASI (Kirim Pertama Kali ATAU Pengingat setelah 6 Menit)
      console.log(`\n🚨 Memproses Notifikasi untuk Node [${sensorId}]! Status: ${currentStatus.toUpperCase()}`);

      // Ambil metadata dari Firestore untuk nama alat & user
      ...
      // Ambil nomor telepon dari Firestore secara dinamis
      let nomorWaTujuan = "";
      ...
      // F. EKSEKUSI KIRIM PESAN VIA GERBANG FONNTE
      const statusPesan = lastSentTime > 0 ? `⚠️ *PENGINGAT ULANG (BELUM AMAN / BELUM DITANGANI)* ⚠️\n\n` : `🚨 *AETHERIS KITCHEN ALERT SYSTEM* 🚨\n\n`;
      ...
      const templatePesanWA = 
        ...
        `🌡️ *Suhu:* ${telemetry.temperature}°C\n\n` +
      ...
      // Simpan/Kunci waktu pengiriman sekarang agar tertahan selama 6 menit ke depan
...
// ====================================================================
// ALUR 2: SINKRONISASI DUA ARAH (RESET PERANGKAT FISIK SAAT DIKLIK DI WEB)
// ====================================================================
...
db.collection('alerts').where('isResolved', '==', true).onSnapshot((snapshot) => {
  snapshot.docChanges().forEach(async (change) => {
    if (change.type === 'modified' || change.type === 'added') {
      ...
      try {
        // Hapus paksa kunci waktu tunggu karena manajer sudah sadar dan merespons via web
        lastNotificationTimeMap.delete(sensorId);
