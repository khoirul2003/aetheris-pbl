...
function SectionHeading({ title, description, compact = false }: { title: string; description?: string; compact?: boolean; }) {
  return (
    <div>
      ...
      {description ? <p className="mt-1 text-xs" style={{ color: "var(--card-text-faint)" }}>{description}</p> : null}
    </div>
  );
}

function ChartPanel({ title, accent, data, labels }: { title: string; accent: "olive" | "rose" | "emerald"; data: number[]; labels: string[]; }) {
  const accentClass = accent === "olive" ? "bg-[var(--accent-primary)]" : accent === "emerald" ? "bg-emerald-500" : "bg-rose-500";
  
  return (
    <div className="flex flex-col rounded-2xl p-5 shadow-sm overflow-hidden w-full h-full" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
      ...
      <div className="w-full overflow-x-auto pb-2 custom-scrollbar mt-auto">
        <div className="flex h-56 min-w-70 items-end gap-2 px-2 pt-4 text-[10px]" style={{ borderBottom: "1px solid var(--chart-axis)", borderLeft: "1px solid var(--chart-axis)", color: "var(--card-text-faint)" }}>
          {data.map((value, index) => (
            <div key={labels[index] ?? index} className="group flex h-full flex-1 flex-col items-center justify-end gap-2">
              <div className="flex w-full items-end justify-center gap-1">
                <div 
                  className={`${accentClass} relative flex-1 rounded-t-md min-w-4 group-hover:opacity-90`} 
                  style={{ height: `${Math.max(8, value)}%` }}
                >
                  <span className="absolute -top-7 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-[#1A1F24] px-2 py-1 font-sans text-[10px] font-bold text-white opacity-0 group-hover:opacity-100 sm:block z-10 shadow-md pointer-events-none">
                    {value}
                  </span>
                </div>
              </div>
              <span className="font-medium truncate w-full text-center" style={{ color: "var(--card-text-muted)" }}>
                {labels[index]}
              </span>
            </div>
          ))}
  ...
  return (
    <AdminLayout title="Dashboard" description="All figures below are fetched directly from Firestore and Realtime Database.">
      <div className="flex w-full flex-col gap-5 h-full">
          <section className="flex flex-col gap-4 rounded-2xl p-5 shadow-sm backdrop-blur" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              ...
              <button className="inline-flex items-center gap-2 self-start rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm cursor-pointer" style={{ borderWidth: 1, borderColor: "var(--accent-primary-border)", backgroundColor: "var(--card-bg-solid)", color: "var(--accent-primary)" }}>
                <Download size={16} />
                Export Data
              </button>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {dashboardStats.map((stat) => {
                ...
                const pillClass = stat.tone === "olive" ? "border-[#4D6344]/30 text-[#4D6344] dark:text-[#8fac7e] bg-[#EAF2EB] dark:bg-[#4D6344]/15 dark:border-[#4D6344]/30" : stat.tone === "emerald" ? "border-emerald-500 text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 dark:border-emerald-500/30" : stat.tone === "amber" ? "border-amber-500 text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 dark:border-amber-500/30" : "border-rose-500 text-rose-700 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 dark:border-rose-500/30";

                return (
                  <div key={stat.label} className={`rounded-xl p-5 border-t-4 ${accentClass}`} style={{ backgroundColor: "var(--card-bg-solid)", borderWidth: 1, borderColor: "var(--card-surface-border)" }}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-[11px] font-bold uppercase tracking-[0.18em]" style={{ color: "var(--card-text-muted)" }}>{stat.label}</p>
          ...
          <section className="grid gap-5 lg:grid-cols-3 items-stretch">
            ...
            <div className="space-y-5 lg:col-span-2 flex flex-col h-full">
              <div className="grid gap-5 xl:grid-cols-[1.6fr_1fr]">
                ...
                <div className="rounded-2xl p-5 shadow-sm" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-500/10 text-rose-500"><AlertTriangle size={18} /></div>
                    ...
                    <button className="inline-flex items-center gap-1 text-sm font-bold hover:opacity-80 cursor-pointer whitespace-nowrap" style={{ color: "var(--accent-primary)" }}>
                      View All
                  ...
                  <div className="mt-5 space-y-3">
                    {realtimeRestaurants.length > 0 ? (
                      realtimeRestaurants.map((item) => {
                        const badgeClass = item.tone === "rose" ? "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400" : "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400";
                        return (
                          <div key={`${item.restaurant}-${item.location}`} className="rounded-xl p-4 shadow-sm" style={{ backgroundColor: "var(--card-bg-solid)", borderWidth: 1, borderColor: "var(--card-surface-border)" }}>
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs" style={{ color: "var(--card-text-muted)" }}>{item.location}</p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${badgeClass}`}>{item.status}</span>
                                    <button className="text-sm font-bold hover:underline cursor-pointer" style={{ color: "var(--accent-primary)" }}>Investigate</button>
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      <div className="rounded-xl border border-dashed p-6 text-center text-sm" style={{ borderColor: "var(--card-surface-border)", backgroundColor: "var(--card-surface)", color: "var(--card-text-muted)" }}>No restaurants currently in danger or warning condition.</div>
                    )}
                  </div>
                </div>
                ...
                <div className="rounded-2xl p-5 shadow-sm" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button style={{ color: "var(--card-text-faint)" }} className="hover:opacity-80"><MoreVertical size={18} /></button>
                  </div>
                  ...
                  <div className="mt-8 pt-5" style={{ borderTop: "1px solid var(--card-surface-border)" }}>
                      ...
                      {Object.entries(monthlyRevenue.byPackage).length > 0 ? (
                        Object.entries(monthlyRevenue.byPackage).slice(0, 6).map(([packageName, amount]) => (
                          <div key={packageName} className="flex flex-col items-center gap-2">
                            <div className="flex h-32 w-full items-end justify-center rounded-xl px-2 py-2" style={{ backgroundColor: "var(--chart-bg)" }}>
                              <div className="w-full rounded-t-lg" style={{ backgroundColor: "var(--chart-bar)", height: `${Math.min(100, Math.max(18, (amount / Math.max(monthlyRevenue.total, 1)) * 100))}%` }} />
                            </div>
                            <span className="text-[11px] font-semibold" style={{ color: "var(--card-text-muted)" }}>{packageName}</span>
                          </div>
                        ))
                      ) : (
                        <div className="col-span-6 rounded-xl border border-dashed px-4 py-8 text-center text-sm" style={{ borderColor: "var(--card-surface-border)", backgroundColor: "var(--card-surface)", color: "var(--card-text-muted)" }}>No payment logs for this month yet.</div>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-5 lg:col-span-1 h-full">
              ...
              <div className="rounded-2xl p-5 shadow-sm shrink-0" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
                ...
                <div className="mt-4 space-y-3">
                  {realtimeRestaurants.length > 0 ? (
                    realtimeRestaurants.slice(0, 3).map((item) => (
                      <div key={item.restaurant} className="rounded-xl p-4" style={{ backgroundColor: "var(--card-surface)", borderWidth: 1, borderColor: "var(--card-surface-border)" }}>
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold shrink-0 ${item.tone === "rose" ? "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400" : "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400"}`}>{item.status}</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="rounded-xl border border-dashed p-5 text-sm" style={{ borderColor: "var(--card-surface-border)", backgroundColor: "var(--card-surface)", color: "var(--card-text-muted)" }}>No active emergencies at the moment.</div>
                  )}
                </div>
              </div>

              <div className="rounded-2xl p-5 shadow-sm flex flex-col shrink-0" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
              ...
              <div className="rounded-2xl p-5 shadow-sm flex flex-col flex-1" style={{ backgroundColor: "var(--card-bg)", borderWidth: 1, borderColor: "var(--card-border)" }}>
                <div className="flex items-center justify-between gap-4 mb-4 shrink-0">
                  <div><SectionHeading title="Offline Sensors" description="Sensors currently disconnected from the network." /></div>
                  <span className="rounded-full border border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/10 px-3 py-1 text-xs font-semibold text-rose-600 dark:text-rose-400 whitespace-nowrap">{offlineSensorRows.length} item(s)</span>
                </div>
                
                <div className="overflow-x-auto custom-scrollbar rounded-xl flex-1" style={{ borderWidth: 1, borderColor: "var(--table-border)" }}>
                  <table className="min-w-full divide-y text-left text-sm whitespace-nowrap" style={{ borderColor: "var(--table-border)" }}>
                    <thead style={{ backgroundColor: "var(--table-head-bg)", color: "var(--card-text-muted)" }} className="sticky top-0">
                      <tr>
                    ...
                    <tbody className="divide-y" style={{ backgroundColor: "var(--table-body-bg)", color: "var(--card-text)", borderColor: "var(--table-border)" }}>
