...
import { ClientProfileModel } from "@/models/clientProfileModel"; // Memanggil model profil untuk lookup nama restoran
...
export default function AdminAnalyticsPage() {
  ...
  // 1. Ambil data nama restoran secara bersih melalui lapisan model profil
  useEffect(() => {
  ...
  // 2. Berlangganan data alert menggunakan model alert proyek
  useEffect(() => {
    const unsubscribeAlerts = ClientAlertModel.subscribeToAlerts("ALL", (snapshotData: AlertData[]) => {
      const frequencies: { [key: string]: number } = {};
      
      snapshotData.forEach((item) => {
        const name = usersMap[item.userId] || item.restaurantName || "";
                ...
                {dynamicChartData.map((data, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end min-w-[20px]">
                    <div className="w-full flex items-end gap-1 h-full">
                      {/* FIX: Pembersihan tag salah ketik < Gentiles menjadi struktur div yang valid */}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-left table-auto">
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                  ...
                  )) : <tr><td colSpan={3} className="text-center text-slate-400 py-10 text-xs">Tidak ditemukan riwayat log kebocoran sensor.</td></tr>}
                </tbody>
              </table>
            </div>
