...
import { ClientAlertModel, AlertData } from "@/models/clientAlertModel"; // Memanggil model terpusat proyek
import { db } from "@/lib/firebase"; // Diperlukan untuk melakukan real-time stream data users map
...
export default function AdminAlertsPage() {
  ...
  // Menggunakan tipe data AlertData dari model proyek
  const [alertsData, setAlertsData] = useState<AlertData[]>([]);
  
  // State Kamus Peta untuk LookUp Data Relasional (userId -> restaurantName)
  const [usersMap, setUsersMap] = useState<{ [uid: string]: string }>({});

  // 1. Ambil data peta restaurantName dari koleksi 'users' secara real-time
  useEffect(() => {
    ...
    const unsubscribeUsers = onSnapshot(usersQuery, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        // Hubungkan document ID (UID) dengan properti restaurantName dari database
  ...
  // 2. Berlangganan data alert secara GLOBAL menggunakan parameter "ALL"
  useEffect(() => {
    // Menggunakan string "ALL" agar query where() di bypass oleh model dan admin bisa memantau semua restoran
  ...
  // Membaca format tanggal ISO dari properti createdAt secara aman
  ...
  // Menyaring data asli secara akurat berdasarkan state filter di bagian atas halaman sebelum di-map ke tabel
  const filteredAlerts = alertsData.filter((item) => {
    ...
    const itemStatus = item.isResolved ? "SELESAI" : "PERLU PERHATIAN";
    
    // Tarik nama restoran secara dinamis dari database usersMap lewat relasi userId
    const restaurantName = usersMap[item.userId] || item.restaurantName || "Memuat Mitra...";
    const sensorName = item.sensorName || item.location || "Sensor Dapur";
    ...
    const timeStr = formatAlertTime(item.createdAt);
    return {
      id: item.userId ? item.userId.substring(0, 7).toUpperCase() : "UID Restoran", // ID diambil dari UID User
      time: timeStr,
      // KONEKSI DINAMIS REAL: Otomatis membaca field restaurantName dari mapping usersMap database
      restaurant: usersMap[item.userId] || item.restaurantName || "",
      sensor: item.sensorName || item.location || "Sensor Dapur",
      level: item.level === "danger" ? "BAHAYA" : "WASPADA",
      action: item.level === "danger" ? "Buzzer Aktif & Notifikasi SMS" : "Kirim Notifikasi Aplikasi",
      status: item.isResolved ? "SELESAI" : "PERLU PERHATIAN",
    };
  });
  ...
  // Fungsi Cetak PDF Report (Merah Gradasi)
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-slate-50 border-b border-t border-slate-200 print:hidden text-slate-700 text-sm">
      ...
      <div className="flex items-center gap-4">
        <span className="text-xs text-slate-500 font-medium">
          Menampilkan {indexOfFirstRow + 1} - {Math.min(indexOfLastRow, filteredAlerts.length)} dari {filteredAlerts.length} log
        </span>
        <div className="flex gap-1">
          <button 
            disabled={currentPage === 1}
            onClick={() => setCurrentPage(prev => prev - 1)}
            className="p-1.5 bg-white border border-slate-300 rounded-lg font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50 disabled:hover:bg-white cursor-pointer transition-colors"
          >
            <ChevronLeft size={16} />
          </button>
          <button 
  ...
  return (
    ...
    >
      ...
      {/* Struktur flex flex-col full width agar Navbar teratur di bagian atas tanpa bertumpuk */}
      <div className="flex flex-col flex-grow min-w-0">
        <Navbar title="Riwayat Alert" />

        {/* pt-24 memastikan letak konten utama turun rapi di bawah tinggi Navbar */}
        <main className="md:ml-64 pt-24 px-8 pb-8 w-auto print:ml-0 print:p-0 transition-all flex-grow">
          
          {/* HEADER SECTION YANG KAMU BERIKAN — 100% Menggunakan Struktur Orisinalmu */}
          <header className="flex justify-between items-start mb-8 border-b border-slate-200 pb-5 print:mb-4 print:pb-2">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1 print:text-xs">Daftar log peringatan bahaya dari seluruh sistem sensor mitra platform Aetheris.</p>
            </div>
          ...
          {/* Panel Kontrol Filter & Pencarian (Langsung berfungsi secara dinamis) */}
